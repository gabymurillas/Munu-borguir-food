...
# Simulación de los datos del menú (basado en lo leído de js/data.js)
# Usaré un diccionario similar para generar el SQL
menu_data = {
    "perros": [
...
sql_output = "-- Data Migration\n\n"

for category_slug, products in menu_data.items():
    sql_output += f"-- Category: {category_slug}\n"
    sql_output += f"DO $$\n"
    sql_output += f"DECLARE\n"
    sql_output += f"    cat_id UUID;\n"
    sql_output += f"    prod_id UUID;\n"
    sql_output += f"BEGIN\n"
    sql_output += f"    SELECT id INTO cat_id FROM categories WHERE slug = '{category_slug}';\n\n"
    
    for p in products:
        name = p['nombre'].replace("'", "''")
        desc = p.get('desc', '').replace("'", "''")
        titulo_card = p.get('tituloCard', '').replace("'", "''")
        is_combo = 'true' if p.get('esCombo', False) else 'false'
        
        sql_output += f"    INSERT INTO products (category_id, name, description, titulo_card, is_combo) \n"
        sql_output += f"    VALUES (cat_id, '{name}', '{desc}', '{titulo_card}', {is_combo}) RETURNING id INTO prod_id;\n"
        
        for price in p['precios']:
            label = price['label'].replace("'", "''")
            monto = price['monto']
            sql_output += f"    INSERT INTO prices (product_id, label, amount) VALUES (prod_id, '{label}', {monto});\n"
        
        sql_output += "\n"
        
    sql_output += f"END $$;\n\n"

with open('supabase_data_migration.sql', 'w') as f:
    f.write(sql_output)
